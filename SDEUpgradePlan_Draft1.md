...
1. Ensure that the most current version of ArcGIS Desktop, Engine Runtime, or Server is installed on the machine you will be administrating the upgrade from. Post 10.1, you can only install an ArcSDE version that is concurrent or older than the program you are running the Upgrade Geodatabase tool from.
...
1. Create backups of the geodatabase.
...
7. Be sure to leave "Perform-Pre-Requisite Check" enabled. This double-checks: there are no other active connections, you have sufficient privileges, the database can support XML columns, and all datasets can be opened. If any prerequisites are not met, the tool terminates.
...
9. If needing a service for client access, install the new version of the ArcSDE application server. Then re-create the service using the sdeservice and sdemon command-line tools (included with the application server install).
...
2. Or we do away with pushing data from publication to maintenance in this way. There are two types of data that actually need to be on a maintenance dataset: directly-edited data and reference data. The cases above are likely reference data for editors being pushed back. This need could be met with relationship classes or joins in the editing MXD to cross-reference tables that wouldn't require version-editing to update.
...
5. Post-10.1, views are automatically spatial if they detect a native geometry type. Also, you can no longer create a spatial view with SDEBinary types except via the sdetable command-line tool.
...
1. RLID loads data from our geoprocessing ETLs. There is a known issue with gisrv112 with geometry storage, but Bob and I have created a workaround for the limited life of that server. Also since both Bob and I can adjust the loading script, we can easily correct any of the problems before or as the occur.
2. EWEB loads RLIDGeo datasets weekly to their own SDE database. I've already been in contact with Jeff Schenck and Mike Pulley, and I'm satisfied that their system & method of loading will continue to work after the upgrades. Still, I've also promised Jeff and Mike that they can make a loading run at a test version of an upgraded RLIDGeo prior to the true upgrade.
...
* Maintenance
  - EugeneGeo: Upgrade GDB & datasets, enable versioned views, change geometry default.
  - LCOGAppliedGeo: Upgrade GDB & datasets, enable versioned views, change geometry default.
    - Question: Is this being used? If not, could retire or replace with wholly new version with no legacy cruft.
  - LCOGGeo: Upgrade GDB & datasets, enable versioned views, change geometry default.
  - Regional: Upgrade GDB & datasets, enable versioned views, change geometry default.
* Publication:
  - Reporting: Already upgraded (new GDB).
  - RLIDGeo: Upgrade GDB & datasets, enable versioned views, change geometry default, dataset geometry changes, *maybe* schema owner change.
* LCOG Dev
  - Development: Upgrade GDB & datasets, enable versioned views, change geometry default, dataset geometry changes, *maybe* schema owner change.
    - Option: Could be retired & spun back up when needed.
  - RLIM: To be retired ASAP.
  - Staging: Upgrade GDB & datasets, enable versioned views, change geometry default, dataset geometry changes, schema owner change.
  - Test: Upgrade GDB & datasets, enable versioned views, change geometry default, dataset geometry changes, no schema owner change.
    - Option: Could be retired & spun back up when needed, or at the least merged into Development.
